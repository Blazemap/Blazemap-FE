...
function ReportDetail({ user, id, onDraft, onClose }: { user: DashboardUser; id: string; onDraft: (dirty: boolean, pending: boolean) => void; onClose?: () => void }) {
  ...
  return <div className="flex min-h-0 flex-1 flex-col bg-white">
    ...
    <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-7 py-6">
      ...
      {report && <article>
        <div className="flex items-start justify-between gap-3"><div><h2 className="text-xl font-extrabold">Report details</h2><p className="mt-2 text-sm font-bold text-primary">{reportStatusLabel(report)}</p></div><span className="grid size-11 place-items-center rounded-sm bg-emerald-50 text-emerald-700"><ShieldCheck size={21} aria-hidden="true" /></span></div>
