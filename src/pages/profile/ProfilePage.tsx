...
function Profile({ initial }: { initial: DashboardUser }) {
  ...
  return <><DraftGuard dirty={dirtyName || dirtyPassword || photo.dirty} pending={pending} />
    <div className="relative min-h-[calc(100dvh-80px)] overflow-hidden bg-[#e8edde] lg:min-h-[calc(100dvh-88px)]">
      <div aria-hidden="true" className="pointer-events-none absolute inset-0 overflow-hidden bg-forest"><picture><source media="(max-width: 640px)" srcSet={rainforestSmall} /><img src={rainforest} alt="" className="size-full object-cover object-center opacity-60" decoding="async" /></picture><div className="absolute inset-0 bg-linear-to-b from-forest/50 via-[#dfe8d5]/85 to-[#eef2e6]" /></div>
      <div className="relative z-10 mx-auto w-full max-w-5xl px-4 py-12 sm:px-6">
      <h1 className="mb-7 text-3xl font-extrabold tracking-tight text-white drop-shadow-sm">Account settings</h1>
      <div className="grid items-start gap-6 md:grid-cols-[220px_minmax(0,1fr)]">
        <nav aria-label="Account sections" className="flex gap-2 rounded-2xl border border-primary/10 bg-white p-2 md:flex-col">{([['profile', 'Profile', UserRound], ['password', 'Change password', KeyRound]] as const).map(([value, label, Icon]) => <button key={value} type="button" disabled={pending} aria-pressed={section === value} aria-controls="account-settings" onClick={() => setSection(value)} className={`flex min-h-12 flex-1 items-center gap-3 rounded-xl px-4 text-left text-sm font-bold ${section === value ? "bg-primary text-white" : "hover:bg-secondary"}`}><Icon size={18} aria-hidden="true" />{label}</button>)}</nav>
