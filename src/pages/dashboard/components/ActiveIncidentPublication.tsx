...
export default function ActiveIncidentPublication({ user, detail, onDraft }: { user: DashboardUser; detail: CaseDetail; onDraft: (state: { dirty: boolean; pending: boolean }) => void }) {
  ...
  return <section aria-labelledby="incident-publication-title" className="mt-5 rounded-xl border border-primary/15 bg-secondary/20 p-4">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><h4 id="incident-publication-title" className="font-extrabold">Publish confirmed incident</h4><p className="mt-1 text-xs leading-5 text-muted-foreground">Confirmation stays private until an authorized administrator reviews the public text and perimeter, then publishes them. Private reports and reporter details are never published here.</p></div><span className="rounded-full bg-white px-2.5 py-1 text-xs font-bold">{state.label}</span></div>
    {publication && !editing && <p className="mt-3 text-sm">{publication.summary}</p>}
    {!editing && user.canPublishInformation && <Button type="button" variant="outline" className="mt-3" onClick={start}>{publication?.status === "DRAFT" ? "Edit incident draft" : publication?.status === "PUBLISHED" ? "Prepare updated draft" : "Prepare public update"}</Button>}
