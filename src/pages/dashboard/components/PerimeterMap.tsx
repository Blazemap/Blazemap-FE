...
function fit(map: Map, points: [number, number][]) {
  ...
  map.fitBounds(bounds, { padding: { left: window.innerWidth >= 768 ? 440 : 32, right: 48, top: 130, bottom: 150 }, maxZoom: 15, duration: 0 });
}
...
export function PerimeterDrawing({ map, draft, setDraft }: { map: Map; draft: PerimeterDraft; setDraft: PerimeterEditorProps["setDraft"] }) {
  ...
  useEffect(() => {
    ...
    const markers = rings[active].map((point, index) => {
      ...
      button.addEventListener("click", event => {
        ...
        if (index === 0) setDraft(current => current ? editDraft(current, { type: "close" }) : current);
      });
      const marker = new Marker({ element: button, draggable: !draft.pending }).setLngLat(point).addTo(map);
      marker.on("dragend", () => {
  ...
  }, [map, draft.drawing, draft.pending, setDraft]);
  useEffect(() => { fit(map, latest.current.drawing.rings.flat()); }, [map, draft.caseId, draft.fit]);
  return <p role={failed ? "alert" : "status"} className="absolute right-4 top-32 z-10 max-w-64 rounded-lg border bg-white p-3 text-xs">{failed ? "Drawing rendering failed. Coordinate inputs remain available." : "Private unsaved perimeter. Public observation selection and report picking are paused."}</p>;
}
