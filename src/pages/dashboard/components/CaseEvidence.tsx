...
export default function CaseEvidence({ user, detail, refresh, onDraft, draft, setDraft, canDraw, reportId, reportReviewStatus }: PerimeterEditorProps & { user: DashboardUser; detail: CaseDetail; refresh: () => void; canDraw: boolean; reportId?: string; reportReviewStatus?: string; onDraft: (state: { dirty: boolean; pending: boolean }) => void }) {
  const [status, setStatus] = useState<string>(!reportId ? "CONFIRMED_FIRE" : reportReviewStatus === "DECLINED" ? "DECLINED" : detail.verification === "CONFIRMED_FIRE" ? "CONFIRMED_FIRE" : reportReviewStatus === "UNDER_REVIEW" || reportReviewStatus === "NEEDS_DETAILS" ? "IN_PROGRESS" : "REVIEWED");
  ...
  return <section className="mt-5 flex flex-col gap-4 border-t pt-4" aria-label="Operator status update">
    {(reportId || detail.verification !== "CONFIRMED_FIRE") && <form className="order-2 space-y-3" onSubmit={event => { event.preventDefault(); mutation.mutate(); }}><fieldset disabled={pending || (!!draft && !confirmation)} className="space-y-3">
      <div className="flex items-center justify-between gap-2"><h4 className="text-sm font-bold">{reportId ? "Review update" : "2. Review and draw fire boundary"}</h4>{reportId && <FieldHelp title="Review and case status">Review updates are for the report owner. Declining a report does not verify that there is no fire. Case handling tracks the response separately; a verified finding requires actual inspection evidence.</FieldHelp>}</div>
      {reportId && <label htmlFor="case-review-status" className="block text-sm font-bold">Status <span aria-hidden="true">*</span><FieldSelect id="case-review-status" required disabled={pending || (!!draft && !confirmation)} value={selectedStatus} onValueChange={selectStatus} options={[["REVIEWED", "Reviewed"], ["IN_PROGRESS", "In progress"], ["CONFIRMED_FIRE", "Confirmed"], ["DECLINED", "Declined"]].map(([value, label]) => ({ value, label, disabled: value !== "CONFIRMED_FIRE" && !reportId }))} /></label>}
      {confirming && <p className="text-xs leading-5 text-muted-foreground">Use the assigned team's actual field observation; the report alone does not confirm fire.</p>}
      ...
      {confirming && <label htmlFor="case-decision-note" className="block text-sm font-bold">Decision note for report owners <span aria-hidden="true">*</span><textarea id="case-decision-note" required aria-required="true" minLength={5} maxLength={2000} className={`${control} py-2`} value={confirmation ? draft.reason : description} onChange={event => { if (confirmation) setDraft(current => current ? { ...current, reason: event.target.value } : current); else setDescription(event.target.value); }} /><FieldLength value={confirmation ? draft.reason : description} min={5} max={2000} /></label>}
      {!confirming && <><p id="case-description-help" className="text-xs text-muted-foreground">Private update for the report owner (5–2,000 characters).</p><label htmlFor="case-review-description" className="block text-sm font-bold">Update for the report owner <span aria-hidden="true">*</span><textarea id="case-review-description" aria-describedby="case-description-help" required aria-required="true" minLength={5} maxLength={2000} className={`${control} py-2`} value={description} onChange={event => setDescription(event.target.value)} /><FieldLength value={description} min={5} max={2000} /></label></>}
