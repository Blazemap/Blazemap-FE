...
import { Button, FieldHelp, FieldSelect } from "@/components/ui";
...
export default function CaseEvidence({ user, detail, refresh, onDraft, draft, setDraft, canDraw, reportId, reportReviewStatus }: PerimeterEditorProps & { user: DashboardUser; detail: CaseDetail; refresh: () => void; canDraw: boolean; reportId?: string; reportReviewStatus?: string; onDraft: (state: { dirty: boolean; pending: boolean }) => void }) {
  ...
  return <section className="mt-5 space-y-4 border-t pt-4" aria-label="Operator status update">
    <form className="space-y-3" onSubmit={event => { event.preventDefault(); mutation.mutate(); }}><fieldset disabled={pending || (!!draft && !confirmation)} className="space-y-3">
      ...
      <p id="case-description-help" className="text-xs text-muted-foreground">Explain the decision or next action in 5–2,000 characters. This message is also used when saving case handling or a verified not-fire finding below.</p>
      <label className="block text-sm font-bold">Update for the report owner <span aria-hidden="true">*</span><textarea aria-describedby="case-description-help" required aria-required="true" minLength={5} maxLength={2000} className={`${control} py-2`} value={confirmation ? draft.reason : description} onChange={event => { if (confirmation) setDraft(current => current ? { ...current, reason: event.target.value } : current); else setDescription(event.target.value); }} /></label>
    ...
    <details className="border-t pt-3"><summary className="min-h-11 cursor-pointer text-sm font-bold">Field evidence (required for verification)</summary>
      ...
      <p className="text-xs">Only verification needs field evidence: visible fire for confirmation, or no indication for a verified not-fire finding. Ordinary review updates do not. Citizen reports and drawings are not substitutes.</p><FieldHelp title="Recorded field evidence">Select a saved inspection, or record one below. The separate boundary source and time must describe the actual boundary observation, not be copied from the review update.</FieldHelp></fieldset>
      <details><summary className="min-h-11 cursor-pointer text-sm font-bold">Record an actual field observation</summary><form className="space-y-3" onSubmit={event => { event.preventDefault(); evidence.mutate(); }}><fieldset disabled={pending || (!!draft && !confirmation)} className="space-y-3">
        ...
        <div className="flex items-center justify-between gap-2"><label htmlFor="case-field-source" className="text-xs font-bold">Evidence source <span aria-hidden="true">*</span></label><FieldHelp title="Evidence source">Name the observing team or inspection record, not the review decision. This identifies where the field evidence came from.</FieldHelp></div>
        <input id="case-field-source" required aria-required="true" minLength={3} maxLength={300} className={control} value={source} onChange={event => setSource(event.target.value)} />
        <p id="case-observed-help" className="text-xs text-muted-foreground">When the inspection happened in your local time, not when you entered this update. Future times are not allowed.</p>
        <label className="block text-xs font-bold">Actual observation time <span aria-hidden="true">*</span><input aria-describedby="case-observed-help" type="datetime-local" required aria-required="true" className={control} value={observed} onChange={event => setObserved(event.target.value)} /></label>
        <p id="case-field-description-help" className="text-xs text-muted-foreground">Record what the inspection found (5–2,000 characters), rather than repeating the review decision.</p>
        <label className="block text-xs font-bold">Inspection findings <span aria-hidden="true">*</span><textarea aria-describedby="case-field-description-help" required aria-required="true" minLength={5} maxLength={2000} className={`${control} py-2`} value={fieldDescription} onChange={event => setFieldDescription(event.target.value)} /></label>
        <p id="case-coordinates-help" className="text-xs">Enter actual observation coordinates, not the report location or polygon center. Coordinates are required for visible-fire and no-indication verification; otherwise optional as a pair.</p>
        <div className="grid grid-cols-2 gap-2"><label className="text-xs font-bold">Observation latitude<input aria-describedby="case-coordinates-help" type="number" step="any" min={-90} max={90} className={control} value={latitude} onChange={event => setLatitude(event.target.value)} /></label><label className="text-xs font-bold">Observation longitude<input aria-describedby="case-coordinates-help" type="number" step="any" min={-180} max={180} className={control} value={longitude} onChange={event => setLongitude(event.target.value)} /></label></div>
