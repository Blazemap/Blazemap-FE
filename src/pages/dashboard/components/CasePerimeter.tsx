...
export default function CasePerimeter({ user, detail, draft, setDraft, refresh, canDraw }: PerimeterEditorProps & { user: DashboardUser; detail: CaseDetail; refresh: () => void; canDraw: boolean }) {
  ...
  return <section className="mt-5 space-y-3 border-t pt-4" aria-label="Private incident perimeter"><h4 className="font-extrabold">Private incident perimeter</h4>
    ...
    {active && <motion.form initial={{ opacity: 0, y: reducedMotion ? 0 : 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: reducedMotion ? 0 : 0.16 }} className="space-y-3" aria-label={detail.perimeter && !active.confirmation ? "Revise saved boundary" : "Save private boundary"} onSubmit={event => {
      ...
      {active.confirmation && <details><summary className="min-h-11 cursor-pointer text-sm font-bold">Advanced: boundary provenance</summary><label className="mt-2 flex items-start gap-2 text-sm"><input type="checkbox" checked={active.confirmation.reuseFieldObservation === true} onChange={event => change({ confirmation: { ...active.confirmation!, reuseFieldObservation: event.target.checked } })} />Use the selected observation’s source and time for this boundary</label>{!active.confirmation.reuseFieldObservation && <><label className="block text-xs font-bold">Perimeter observed at (local time) <span aria-hidden="true">*</span><input required aria-required="true" type="datetime-local" className={control} value={active.observedAt} onChange={event => change({ observedAt: event.target.value })} /></label><label className="block text-xs font-bold">Perimeter source <span aria-hidden="true">*</span><input required aria-required="true" minLength={3} maxLength={300} className={control} value={active.source} onChange={event => change({ source: event.target.value })} /><FieldLength value={active.source} min={3} max={300} /></label></>}</details>}{!active.confirmation && <label className="block text-xs font-bold">Change reason <span aria-hidden="true">*</span><textarea required aria-required="true" minLength={5} maxLength={2000} className={`${control} py-2`} value={active.reason} onChange={event => change({ reason: event.target.value })} /><FieldLength value={active.reason} min={5} max={2000} /></label>}{!active.confirmation && <label className="block text-xs font-bold">Operator-supplied authority note <span aria-hidden="true">*</span><input required aria-required="true" minLength={3} maxLength={500} className={control} value={active.authority} onChange={event => change({ authority: event.target.value })} /><FieldLength value={active.authority} min={3} max={500} /></label>}
      {!active.confirmation && <label className="block text-xs font-bold">Explanation for report owners *<textarea required minLength={5} maxLength={2000} className={`${control} py-2`} value={ownerMessage} onChange={event => setOwnerMessage(event.target.value)} /><FieldLength value={ownerMessage} min={5} max={2000} /><span className="block font-normal">Visible only to linked report owners, not public News. Exclude private evidence and personal details.</span></label>}
