...
export default function CaseAnalysis({ user, detail, disabled, onDraft }: { user: DashboardUser; detail: CaseDetail; disabled: boolean; onDraft: (state: { dirty: boolean; pending: boolean }) => void }) {
  const run = useGovernmentMutation(user, () => requestCaseAnalysis(detail.id));
  ...
  return <section aria-label="Case analysis" className="mt-5 space-y-3 border-t pt-4">
    ...
    <p className="text-xs">Evidence strength and potential impact if this is a fire are separate assessments. AI does not verify a fire or authorize action. Running analysis may use the configured paid provider.</p>
