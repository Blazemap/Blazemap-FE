...
function Workspace({ user }: { user: DashboardUser }) {
  ...
  const detail = caseId ? <CasePanel key={caseId} user={user} id={caseId} draft={perimeterDraft} setDraft={setPerimeterDraft} canDraw={!feed} onWind={setWind} onDraft={onCaseDraft} /> : report ? (reportDetail.initialLoading ? <GovernmentReportDetailSkeleton /> : <>{reportDetail.failed && <div role="alert" className="space-y-3 p-4 text-sm"><p>Report details could not refresh.</p><Button variant="outline" onClick={reportDetail.retry}>Retry</Button></div>}<fieldset disabled={reportsUnavailable} aria-busy={reportDetail.refreshing}><ReportReview key={report.id} user={user} report={report} caseEvidence={reportCase.data?.fieldUpdates ?? []} caseAssignments={reportCase.data?.assignments ?? []} caseVersion={reportCase.data?.version} onDraft={onDraft} perimeterDraft={reportDrawing ? perimeterDraft : null} setPerimeterDraft={setPerimeterDraft} canDraw={!feed && (!perimeterDraft || !!reportDrawing)} /></fieldset></>) : selected ? <ItemDetail item={selected} /> : null;
  return <IncidentPointContext.Provider value={{ start: setIncidentPick, available: !feed && !associationPick && !reviewDraft.pending && !caseDraft.pending }}><ReportAssociationContext.Provider value={{ activeSourceId: associationPick?.sourceId ?? null, available: !feed && !incidentPick && !perimeterDraft && !reviewDraft.pending && !caseDraft.pending, start: pick => { setListOpen(false); setWorklistOpen(false); setCasesOpen(false); setAssociationPick(pick); }, cancel: () => setAssociationPick(null) }}><main className="relative isolate h-dvh overflow-hidden bg-secondary/40 text-forest">
    ...
    {incidentPick && <div className="absolute bottom-5 left-4 z-50 max-w-sm space-y-2 rounded border bg-white p-4 shadow-lg"><p className="text-sm">Click the actual incident point. Boundary draft retained.</p><p className="text-xs">{incidentPick.latitude && incidentPick.longitude ? `${incidentPick.latitude}, ${incidentPick.longitude}` : "No point selected"}</p><Button disabled={!incidentPick.latitude || !incidentPick.longitude} onClick={() => { incidentPick.apply(incidentPick); setIncidentPick(null); }}>Use point</Button><Button variant="outline" onClick={() => setIncidentPick(null)}>Cancel point selection</Button></div>}
