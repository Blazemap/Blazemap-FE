...
export function DonutChart({ title, subtitle, total, parts, href, help }: { title: string; subtitle: string; total: number; parts: ChartPart[]; href?: string; help?: string }) {
  let offset = 0;
  ...
  return <section aria-label={title} className={`${panelClass} flex flex-col p-5 sm:p-6`}><div className="flex items-center justify-between gap-2"><h2 className="text-base font-extrabold">{title}</h2>{help && <FieldHelp title={title}>{help}</FieldHelp>}</div><p className="mt-1 text-xs text-muted-foreground">{subtitle}</p><div className="mt-6 grid flex-1 items-center gap-6 sm:grid-cols-[150px_1fr] xl:grid-cols-1 2xl:grid-cols-[150px_1fr]"><div className="relative mx-auto size-36 rounded-full" style={{ background: `conic-gradient(${stops})` }} aria-hidden="true"><div className="absolute inset-5 grid place-items-center rounded-full bg-white text-center"><span><strong className="block text-3xl font-extrabold tabular-nums">{total}</strong><span className="text-[11px] font-bold text-muted-foreground">total cases</span></span></div></div><div><p className="sr-only">{total} cases in total.</p><ul className="space-y-3">{parts.map(part => <li key={part.label} className="flex items-center justify-between gap-4 text-sm"><span className="flex min-w-0 items-center gap-2"><span className="size-2.5 shrink-0 rounded-sm" style={{ background: part.color }} aria-hidden="true" /><span className="font-semibold">{part.label}</span></span><span className="font-extrabold tabular-nums">{part.value}</span></li>)}</ul></div></div>{href && <Link to={href} className="mt-5 inline-flex min-h-11 items-center justify-end gap-2 border-t border-primary/10 pt-4 text-xs font-extrabold text-primary hover:underline">View details <ArrowUpRight size={14} aria-hidden="true" /></Link>}</section>;
}

export function BarChart({ title, subtitle, parts, href, help }: { title: string; subtitle: string; parts: ChartPart[]; href?: string; help?: string }) {
  const maximum = Math.max(1, ...parts.map(part => part.value));
  return <section aria-label={title} className={`${panelClass} flex flex-col p-5 sm:p-6`}><div className="flex items-center justify-between gap-2"><h2 className="text-base font-extrabold">{title}</h2>{help && <FieldHelp title={title}>{help}</FieldHelp>}</div><p className="mt-1 text-xs text-muted-foreground">{subtitle}</p><div className="mt-6 flex-1 space-y-4">{parts.map(part => <div key={part.label}><div className="mb-2 flex items-center justify-between gap-4 text-xs"><span className="font-bold">{part.label}</span><span className="font-extrabold tabular-nums">{part.value}</span></div><div className="h-2.5 overflow-hidden rounded-full bg-secondary" aria-hidden="true"><div className="h-full rounded-full" style={{ width: `${part.value / maximum * 100}%`, background: part.color }} /></div></div>)}</div>{href && <Link to={href} className="mt-5 inline-flex min-h-11 items-center justify-end gap-2 border-t border-primary/10 pt-4 text-xs font-extrabold text-primary hover:underline">View details <ArrowUpRight size={14} aria-hidden="true" /></Link>}</section>;
}
