...
export const panelClass = "rounded-xl border border-primary/10 bg-white shadow-[0_18px_50px_-42px_rgba(23,59,43,0.9)]";
...
export function DonutChart({ title, subtitle, total, parts, href }: { title: string; subtitle: string; total: number; parts: ChartPart[]; href?: string }) {
  let offset = 0;
  const stops = total > 0 ? parts.map(part => { const start = offset; offset += part.value / total * 100; return `${part.color} ${start}% ${offset}%`; }).join(",") : "#e6eddf 0% 100%";
