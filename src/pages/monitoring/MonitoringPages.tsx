...
import { FileClock, Flame, Layers3, RadioTower, Search, ShieldAlert, X } from "lucide-react";
...
import { MonitoringAvatar } from "./MonitoringAvatar";
...
function sourceState(status: SourceHealth["sources"][number]["status"]) {
  ...
  if (status === "ON_DEMAND") return { label: "On demand", tone: "info" as const };
...
export function SourceHealthRow({ source }: { source: SourceHealth["sources"][number] }) {
  ...
  return <li className="space-y-2 py-4">
    ...
    <p className="text-xs text-muted-foreground">{source.id === "GOOGLE_WEATHER" ? source.lastSuccessAt ? <>Last successful request: <time dateTime={source.lastSuccessAt}>{formatTime(source.lastSuccessAt)}</time></> : "Current weather is requested when a case is opened; no request recorded yet." : source.lastSuccessAt ? <>Last successful sync: <time dateTime={source.lastSuccessAt}>{formatTime(source.lastSuccessAt)}</time></> : "No successful sync recorded."}</p>
    {source.id === "GOOGLE_WEATHER" && attentionStatuses.has(source.status) && <p className="text-xs leading-5">Current weather is requested at case coordinates. Check that the case has coordinates; if the provider is not configured or unavailable, weather and downwind attention cannot be shown.</p>}
  </li>;
}
...
function summaryCharts(data: NonNullable<ReturnType<typeof useMonitoringSummary>["data"]>) {
  ...
  const priority: ChartPart[] = [
    { label: "Critical", value: data.cases.byPriority.CRITICAL, color: "#b91c1c" },
...
function DetailPath({ id, query, children }: { id: string; query: string; children: ReactNode }) {
  ...
  return <Link to={to} className="inline-flex min-h-11 items-center justify-end text-xs font-extrabold text-primary hover:underline">{children}</Link>;
}
...
export function OverviewPage() {
  ...
  const queue = [...(reports.data?.data ?? [])].filter(report => report.reviewStatus !== "DECLINED").slice(0, 5);
  const loading = summary.loading || health.loading || reports.loading;
  const retry = () => { summary.retry(); health.retry(); reports.retry(); };
  return <>
    <PageIntro eyebrow="Overview" title="See what needs attention now." description="Current report queues, case status, workload distribution, and source health in one operational view." />
    ...
    <section className="mt-7"><SectionHeading title="Needs attention" description="Current records that may require review or follow-up." />{summary.initialLoading && !summary.data ? <StatSkeletons /> : <div className="grid gap-4 sm:grid-cols-2 2xl:grid-cols-4"><StatCard label="Reports awaiting review" value={summary.data?.reports.awaitingReview ?? 0} detail={`${summary.data?.reports.inProgress ?? 0} additional reports are in progress.`} icon={FileClock} attention /><StatCard label="Unverified open cases" value={summary.data?.cases.openUnverified ?? 0} detail={`${summary.data?.cases.highPriorityOpen ?? 0} high or critical cases across all open cases.`} icon={ShieldAlert} attention /><StatCard label="Confirmed open cases" value={summary.data?.cases.openConfirmed ?? 0} detail={`${summary.data?.cases.activeHandling ?? 0} cases are in active handling stages across all cases.`} icon={Flame} /><StatCard label="Sources needing attention" value={sourceAttention} detail="FIRMS, Google Weather, AI, database, and uploads." icon={RadioTower} attention /></div>}</section>
    {(summary.initialLoading || charts) && <section className="mt-7"><SectionHeading title="Case and report status" description={summary.data ? `Counts across the system as of ${formatTime(summary.data.asOf)}. Use these charts to identify cases and reports needing follow-up.` : "Loading case and report counts."} />
{summary.initialLoading && !summary.data ? <DistributionSkeleton /> : charts && <div className="grid gap-5 xl:grid-cols-2 2xl:grid-cols-4"><DonutChart title="Case verification" subtitle="All cases · share by verification result" total={summary.data!.cases.total} parts={charts.verification} href="/monitoring/cases" help="Shows how many cases have not been verified, were confirmed as fires, or were ruled out. Use it to see how much verification work remains. Closed cases are included." /><BarChart title="Open-case priority" subtitle="Open cases only · count by priority" parts={charts.priority} href="/monitoring/cases" help="Compare the number of open cases at each priority level to decide what needs review first. Closed cases are excluded. Every bar starts at zero and is scaled to the largest category in this chart." /><BarChart title="Case handling" subtitle="All cases · count by handling stage" parts={charts.handling} href="/monitoring/cases" help="Shows where cases are in the handling workflow, including closed cases. Use it to see the current workload by stage. Bars are scaled to the largest category in this chart." /><BarChart title="Report workflow" subtitle="All citizen reports · count by review stage" parts={charts.reports} href="/monitoring/reports" help="Shows how many citizen reports are awaiting review, under review, need details, were reviewed, or were declined. Use it to identify the review backlog. Bars are scaled to the largest category in this chart." /></div>}
</section>}
    <div className="mt-7 grid items-start gap-5 xl:grid-cols-[1.25fr_0.75fr]"><section className={`overflow-hidden ${panelClass}`}><header className="flex items-end justify-between gap-4 border-b border-primary/10 px-5 py-4 sm:px-6"><div><h2 className="text-base font-extrabold">Priority report queue</h2><p className="mt-1 text-xs text-muted-foreground">Server-ranked priority across matching reports.</p></div><Link to="/monitoring/reports" className="text-xs font-extrabold text-primary hover:underline">View reports</Link></header>{reports.initialLoading && !reports.data ? <QueueSkeleton /> : !queue.length ? <EmptyPanel>No reports are available.</EmptyPanel> : <ol className="divide-y divide-primary/10">{queue.map(report => { const appearance = triageAppearance[effectiveReportPriority(report)]; return <li key={report.id}><Link to={`/monitoring/reports/${encodeURIComponent(report.id)}`} className="grid gap-4 px-5 py-4 hover:bg-secondary/30 sm:grid-cols-[1fr_auto] sm:px-6"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="text-sm font-extrabold">{report.number}</span><span className="rounded-full px-2 py-0.5 text-xs font-extrabold" style={{ color: appearance.color, backgroundColor: `${appearance.color}12` }}>{appearance.label}</span><span className="text-xs text-muted-foreground">{reportStatusLabel(report)}</span></div><p className="mt-2 line-clamp-2 text-sm leading-6">{report.description}</p><p className="mt-2 truncate text-xs text-muted-foreground">{reportLocation(report)} · {formatTime(report.observedAt)}</p></div><div className="flex gap-1">{report.observationTypes.map(type => <img key={type} src={`/icons8-${observationAppearance[type].icon}.png`} alt={observationAppearance[type].label} width={26} height={26} className="size-6 object-contain" />)}</div></Link></li>; })}</ol>}</section>
