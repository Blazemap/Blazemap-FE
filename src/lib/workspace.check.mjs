...
for (const viewport of [{ width: 1024, height: 768 }, { width: 1440, height: 900 }, { width: 320, height: 400 }]) {
  for (const box of [{ x: -1000, y: -1000, width: 1, height: 1 }, { x: 9999, y: 9999, width: 9999, height: 9999 }]) {
    ...
    assert.ok(bounded.x + bounded.width <= viewport.width);
    assert.ok(bounded.y + bounded.height <= viewport.height);
...
const mapPanel = await readFile(new URL("../pages/dashboard/components/MapPanel.tsx", import.meta.url), "utf8");
const mapLayers = await readFile(new URL("../pages/dashboard/components/MapLayers.tsx", import.meta.url), "utf8");
assert.match(feed, /<MapPanel title="List"/);
assert.match(feed, /<MapPanel title="Worklist"/);
assert.match(mapLayers, /<FieldSelect/);
assert.match(mapPanel, /setPointerCapture/);
assert.match(mapPanel, /onPointerCancel=\{end\}/);
assert.match(mapPanel, /onKeyDown=\{event => keyboard\(event, true\)\}/);
...
assert.match(mapPanel, /useState\(68\)/);
assert.match(mapPanel, /boundPanel/);
assert.doesNotMatch(feed, /perimeterDraft \? <GovernmentWorklist/);
assert.match(feed, /user.role === "USER" \? <CitizenDashboard/);
assert.match(mapPanel, /opener.current.*focus\(\)/);
...
assert.match(mapPanel, /Math.max\(34, Math.min\(86/);
assert.match(citizenDashboard, /stiffness: 300, damping: 30/);
assert.match(citizenDashboard, /useReducedMotion\(\)/);
assert.match(citizenDashboard, /<MapLayers/);
assert.doesNotMatch(citizenDashboard, /<select|Close the report panel/);
...
assert.doesNotMatch(feed, /aria-label="Feed category"/);
assert.match(feed, /aria-label="Citizen reports feed"/);
assert.match(feed, /<DraftGuard dashboard dirty=\{!!perimeterDraft \|\| caseDraft.dirty\} pending=\{!!perimeterDraft\?\.pending \|\| caseDraft.pending\}/);
...
const dashboardAST = ts.createSourceFile('CitizenDashboard.tsx', citizenDashboard, ts.ScriptTarget.Latest, true, ts.ScriptKind.TSX);
const workspace = dashboardAST.statements.find(node => ts.isFunctionDeclaration(node) && node.name.text === 'CitizenDashboard');
...
for (const dirty of [false, true]) for (const pending of [false, true]) {
  ...
  assert.equal(check({ currentLocation: { pathname: '/dashboard', search: '?panel=report' }, nextLocation: { pathname: '/dashboard', search: '?view=feed' } }), pending || dirty);
...
assert.match(citizenDashboard, /<ReportsPage user=\{user\} id=\{params.get\("report"\)\} onDraft=\{handleDetailDraft\}/);
...
assert.match(flame, /item\.kind !== "publication" \|\| !hasPoint\(item\)/);
assert.match(flame, /flameImage\(\(\) => map.triggerRepaint\(\)\)/);
