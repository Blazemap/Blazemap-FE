...
export async function getDashboardUser(signal: AbortSignal): Promise<DashboardUser> {
  ...
  return { id: user.id, name: user.name, email: user.email, image, role: user.role as DashboardUser["role"], canConfirmIncidents: user.role === "ADMIN" && user.canConfirmIncidents === true, canPublishInformation: user.role === "ADMIN" && user.canPublishInformation === true };
}
