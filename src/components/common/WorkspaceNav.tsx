...
import { ListFilter, MapPin, Newspaper } from "lucide-react";
...
export function WorkspaceNav({ user, children }: { user: DashboardUser; children?: ReactNode }) {
  ...
  const news = user.role === "USER" && new URLSearchParams(search).get("view") === "news";
  const destination = (view: "map" | "feed" | "news") => {
    ...
    <div className="flex min-w-0 items-center justify-between gap-2 sm:flex-1">
      <div className="flex min-w-0 shrink items-center gap-2 sm:gap-3">
        ...
        <nav aria-label="Dashboard views" className="flex rounded-full border border-primary/10 bg-secondary/60 p-0.5 shadow-inner sm:p-1">
          {([["map", "Map", MapPin] as const, ["feed", "Feed", ListFilter] as const, ...(user.role === "USER" ? [["news", "News", Newspaper] as const] : [])]).map(([view, label, Icon]) => {
            const active = view === "news" ? news : view === "feed" ? feed : dashboard && !feed && !news;
            return <Link key={view} to={destination(view)} aria-current={active ? "page" : undefined} className={`flex h-8 min-w-11 items-center justify-center gap-1 rounded-full px-1.5 text-[9px] font-extrabold transition-colors sm:min-w-[74px] sm:gap-1.5 sm:px-3 sm:text-[11px] ${active ? "bg-forest text-white shadow-sm" : "text-muted-foreground hover:bg-white hover:text-forest"}`}><Icon size={14} strokeWidth={2.5} aria-hidden="true" /><span>{label}</span></Link>;
          })}
        </nav>
      </div>
      <div className="ml-auto flex min-w-0 shrink items-center gap-1"><NotificationBell user={user} /><AccountMenu user={user} /></div>
    </div>
