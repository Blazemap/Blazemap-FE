...
export function DraftGuard({ dirty, pending = false, dashboard = false }: { dirty: boolean; pending?: boolean; dashboard?: boolean }) {
  ...
  const blocker = useBlocker(({ currentLocation, nextLocation }) => !loggedOut.current && (pending || (dirty && !(dashboard && ["/dashboard", "/monitoring"].includes(currentLocation.pathname) && nextLocation.pathname === currentLocation.pathname))));
