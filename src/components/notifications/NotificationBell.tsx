...
export function NotificationBell({ user }: { user: DashboardUser }) {
  ...
  function open(item: typeof notifications.items[number]) {
    ...
    if (item.type === "NEARBY_CONFIRMED_FIRE") navigate("/dashboard");
    else if (item.publication) navigate(publicationPath(item.publication.slug, item.type === "NEARBY_COMPLETION" ? "news" : "feed"));
    else if (item.reportId) navigate(user.role === "ADMIN" ? `/dashboard?report=${encodeURIComponent(item.reportId)}` : `/dashboard?panel=my-reports&report=${encodeURIComponent(item.reportId)}`);
  }
