...
const panel = read("MapPanel");
assert.match(citizen, /<MapPanel title=\{selected\?\.title/);
assert.match(panel, /<header className="flex shrink-0[^"]*bg-white/);
assert.match(panel, /onPointerDown=\{event => start\(event\)\}/);
assert.match(panel, /onKeyDown=\{event => keyboard\(event\)\}/);
assert.match(panel, /setPointerCapture/);
assert.match(panel, /boundPanel/);
...
assert.match(panel, /height: `\$\{drawing \? Math\.min\(height, 38\) : height\}dvh`/);
